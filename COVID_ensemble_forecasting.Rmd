...
vic <- samples %>% filter(state=="VIC", .model=="gar")
origins <- sort(unique(vic$forecast_origin))
for(i in seq_along(origins)) {
  p <- vic %>% 
    filter(forecast_origin == origins[i]) %>%
    mutate(dist = dist_sample(sample)) %>%
    select(-sample) %>%
    as_fable(index = date, key=forecast_origin, 
             response = "n", distribution = dist) %>%
    autoplot(level = c(50,70,90), point_forecast = lst(median)) +
    autolayer(filter(localcases, state=="VIC", 
                     date <= origins[i]+42, date >= "2020-09-01"), n) +
      scale_x_date(
...
\centerline{\animategraphics[controls,buttonsize=0.3cm,width=14cm]{10}{figs/ensemble}{1}{36}}
